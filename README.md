...
📊 Interactive Dashboard: You can visit my Tableau Public page to explore the live interactive dashboard.
...
The 4 different .csv datasets used in the project were joined in Tableau to form a Relational Data Model. The dataset consists of 12 main columns obtained by joining 4 relational CSV files (orders, order_details, pizzas, pizza_types):

1. Table and Column Structure (Data Dictionary)
📄 orders.csv
Contains high-level timestamp and ID information for orders.
...
📄 order_details.csv
Contains line-item details and quantities for each order.
...
📄 pizzas.csv
Contains pizza size and price definitions.

pizza_id: Unique identifier for each pizza type and size combination (Primary Key).
pizza_type_id: Foreign key linking each pizza to its broader pizza type (Foreign Key -> pizza_types.pizza_type_id).
size: Size of the pizza (S, M, L, XL, XXL).
price: Unit price of the pizza in USD ($).
📄 pizza_types.csv
Contains menu details including name, category, and ingredients.
...
ingredients: Comma-delimited list of ingredients (All pizzas include Mozzarella Cheese and Tomato Sauce unless specified otherwise).
...
orders ──> order_details (1 to N): Joined on orders.order_id = order_details.order_id
order_details ──> pizzas (N to 1): Joined on order_details.pizza_id = pizzas.pizza_id
pizzas ──> pizza_types (N to 1): Joined on pizzas.pizza_type_id = pizza_types.pizza_type_id
🎯 Key Metrics & Executive Summary
Total Revenue	Total Orders	Total Pizzas Sold
$1,063,618	13,149	64,979
...
Dynamic Quadrant Segmentation: In the price elasticity matrix, reference lines were fixed to the vertical axis for average unit price ($16.48) and the horizontal axis for average units sold, dynamically splitting the product portfolio into 4 quadrants.

📊 Detailed Dashboard Analysis

📈 Page 1: Executive Overview (Performance & Trends)

<img width="1357" height="694" alt="Pizza Sales Overview" src="https://github.com/user-attachments/assets/8ee40190-fe0e-4c58-aa32-bec51adcd089" />

📅 1. Monthly & Daily Trends
Volume and Revenue Peaks: July represents the highest order volume with 1,935 orders, whereas November reached the annual revenue peak at 96,283 dollar. October was the lowest-performing month (83,547 dollar / 1,646 orders).
Daily Volume & AOV Paradox: Friday is the busiest day of the week with 3,538 orders but records the lowest Average Order Value at 47.18 dollar AOV. Conversely, Tuesday generates the highest average basket value at 52.17 dollar AOV despite lower order counts.

⏰ 2. Hourly Volume & Revenue Anomaly (Basket Depth Analysis)
Time Slot	Total Orders	Total Pizzas Sold	Basket Depth	Total Revenue
12:00 (Lunch Shift)	1,327	14,742	11.1 Pizzas / Order	241,663 dollar
18:00 (Dinner Shift)	1,705	4,665	2.7 Pizzas / Order	75,860 dollar
...
After-Work Rush (17:00 - 18:00): Although transaction traffic rises to 1,705 orders, basket depth drops to 2.7 pizzas (individual/family purchases), keeping total volume at 4,665 pizzas and revenue at 75,860 dollar.
...
<img width="1356" height="696" alt="Product Performance" src="https://github.com/user-attachments/assets/c0435064-d10e-4c97-be38-53edaf4bcc17" />

🎯 1. Price vs. Volume Matrix
Star / Flagships (High Price & High Volume): The Barbecue Chicken ($16.75 / 6,198 units) and The California Chicken (16.75 dollar / 5,285 units) lead the menu as high-margin, high-volume star performers.
...
Gourmet / Niche Products (High Price & Low Volume): The Brie Carre (23.65 dollar) and The Greek Pizza (21.00 dollar / 162 units) exhibit high price with low volume, making them primary candidates for menu optimization (recipe or pricing revision).

🛒 Page 3: Market Basket Analysis (Co-Occurrence)

<img width="1362" height="697" alt="Basket Analysis" src="https://github.com/user-attachments/assets/5642a61d-1fd9-4484-b845-72768bd00309" />

🤝 1. Co-Occurrence Matrix
Top Pair: Hawaiian & Thai Chicken were ordered together 270 times, generating 4,436.69 dollar in revenue.
...
Corporate Lunch Packages (12:00 - 13:00): Introduce tailored "Corporate / Catering Department Bundles" for the lunch hour to capitalize on high basket depth (11.1 pizzas/order) and lock in high-revenue orders.

Evening Up-Selling Campaigns (17:00 - 18:00): To convert high dinner transaction traffic (1,705 orders) into higher revenue, deploy "50% Off Second Pizza" or side-add-on promotions to expand basket depth beyond 2.7 pizzas.

Strategic Menu Bundling: Pair Pepperoni as a discounted add-on alongside flagship items (BBQ / Cali Chicken), and re-evaluate pricing or recipe composition for low-volume niche items like The Greek Pizza.
